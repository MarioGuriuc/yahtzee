...
class YahtzeeAI:
  ...
  def choose_category(self, state):
    ...
    return best_category.name if best_category is not None else None

  def choose_hold(self, dice_on_table: list[int], dice_held: list[int]) -> list[int]:
...
class Yahtzee:
  ...
  def score(self, category: Category):
    ...
    print(f"Player {self.state.turn + 1} scored {score} in category {category.name}")
  ...
  def get_player_choice(self):
    if self.state.turn == 1:
      choice = self.ai.choose_action(self.state)
      print("AI chose:", choice)
    else:
    ...
    match choice:
      case "hold":
        if self.state.turn == 0:
          ...
          dice_to_hold = self.ai.choose_hold(self.state.dice_on_table, self.state.dice_held)
          for die in dice_to_hold:
            self.hold(self.state.dice_on_table.index(die))
      case "release":
        index = int(input("Enter index: "))
        self.release(index)
      case "score":
    ...
    while self.state.state_type != StateType.FINAL:
      match self.state.state_type:
        ...
        case StateType.SCORING:
          ...
          category = input("Enter category: ") if self.state.turn == 0 else self.ai.choose_category(self.state)
